...
**Retrieved**: 2026-04-05 (via agent-browser)
...
As listed above, in the context of agents and coding tools, we also have the two popular terms *agent harness* and *(agentic) coding harness*. A coding harness is the software scaffold around a model that helps it write and edit code effectively. An agent harness is a bit broader and not specific to coding (e.g., think of OpenClaw). Codex and Claude Code can be considered coding harnesses.
...
Today, when using LLMs, this layer shapes most of the user experience compared to prompting the model directly or using a web chat UI (which is closer to "chat with uploaded files").
...
The "Stable prompt prefix" means that the information contained there doesn't change too much. It usually contains the general instructions, tool descriptions, and the workspace summary. We don't want to waste compute on rebuilding it from scratch in each interaction if nothing important has changed.
...
> *Related: "A Visual Guide to Attention Variants in Modern LLMs" — Sebastian Raschka, PhD, Mar 22*
...
Following step 4 in the figure above, the latest user request, together with the LLM response and tool output, would then be recorded as a "new event" in both the full transcript and working memory in the next round (not shown to reduce clutter).
...
## Author's Book Announcement (context only)

The post also announces that the author finished writing *Build A Reasoning Model (From Scratch)* (Manning, ~528 pages, early access). Main topics:
...
## Notable Comments (context only)

- **Aditya Sharan**: Jokingly asks whether Raschka read the "Claude Code source" before writing this, given similar timing.
- **Benjamin Riley**: Asks whether the author has thoughts on which domains work well with this agentic architecture beyond coding — suggests coding is easier to "bind/harness" due to its deterministic nature, while OpenClaw-style open-ended tasks may be less reliable. Raises the "specialized vs. general intelligence" debate.
